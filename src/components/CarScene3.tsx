...
const Lamppost = ({ position, color }) => {
  return (
    <group position={position} castShadow>
      ...
      <mesh position={[0, 4, 0.3]} rotation={[Math.PI/2, 0, 0]} castShadow>
...
const Road = () => {
  return (
    <group>
      ...
      <CuboidCollider position={[-5.5, 1, 0]} args={[0.5, 2, 50]} />
      <CuboidCollider position={[5.5, 1, 0]} args={[0.5, 2, 50]}/>
    </group>
  );
};
...
<Sky 
  ...
  />
      ...
      {Array.from({ length: 10 }).map((_, i) => (
        <Lamppost key={`lamp-left-${i}`} position={[-4, 0, -40 + i * 10]} color={"yellow"}  />
      ))}
      
      {Array.from({ length: 10 }).map((_, i) => (
        <Lamppost key={`lamp-right-${i}`} position={[4, 0, -35 + i * 10]} color={"white"} />
      ))}
